...
# --- 2. DATA ENGINE ---
@st.cache_resource(show_spinner="Booting Lightweight Climate Engine...")
def load_climate_data():
    # 🎯 TARGETING THE COMPRESSED FILE
    file_path = "dataset_lite.nc"
    if not os.path.exists(file_path):
        st.error(f"🚨 CRITICAL: '{file_path}' not found. Did you run compress.py?")
        st.stop()
...
try:
    ...
    GLOBAL_CITIES = {
        "Amazon Rainforest": (-3.0, -60.0), "Beijing, China": (39.9, 116.4),
        "Buenos Aires, Argentina": (-34.6, -58.4), "Cape Town, South Africa": (-33.9, 18.4),
        "Delhi, India": (28.6, 77.2), "Dubai, UAE": (25.2, 55.3),
        "London, UK": (51.5, -0.1), "Los Angeles, USA": (34.0, -118.2),
        "Moscow, Russia": (55.7, 37.6), "Mumbai, India": (19.0, 72.8),
        "Nairobi, Kenya": (-1.3, 36.8), "New York, USA": (40.7, -74.0),
        "Paris, France": (48.8, 2.3), "Rio de Janeiro, Brazil": (-22.9, -43.2),
        "Sahara Desert": (23.5, 12.0), "Singapore": (1.3, 103.8),
        "Sydney, Australia": (-33.8, 151.2), "Tokyo, Japan": (35.6, 139.6)
    }
    ...
    st.sidebar.number_input("Latitude", value=st.session_state.lat, step=0.5, key="sidebar_lat", on_change=sync_sidebar)
    st.sidebar.number_input("Longitude", value=st.session_state.lon, step=0.5, key="sidebar_lon", on_change=sync_sidebar)
    ...
    with st.spinner("Rendering Visual Slice..."):
        ...
        # Override the raw NetCDF name to fix the colorbar title
        data_slice.name = units[param]

    fig = px.imshow(
        data_slice,
        x=data_slice.lon, # 🎯 THIS ALIGNS THE MAP TO TRUE COORDINATES
        y=data_slice.lat, # 🎯 THIS ALIGNS THE MAP TO TRUE COORDINATES
    ...
    # Hide the background grid but keep the true spatial mapping
    ...
    # --- 5. BOTTOM SECTION: STATS & GRAPH ---
    st.divider()
    c1, c2 = st.columns([1, 2])
    
    # Extract data for the graph safely
    ...
    c1.metric(f"Current {param}", f"{current_val:.2f} {metric_unit}")
    c1.metric("Timeline Peak", f"{float(point_series.max()):.2f} {metric_unit}")
    
    # Clean the dataframe for a perfect graph
    ...
    trend_fig = px.line(
        ...
        y=param, 
        markers=True, 
        title=f"Historical {param} Trend for selected location"
    )
    trend_fig.update_traces(line=dict(color="#00ffff", width=3), marker=dict(size=6, color="#ff0055"))
    trend_fig.update_layout(height=250, template="plotly_dark", margin={"l": 0, "r": 0, "b": 0, "t": 30}, yaxis_title=units[param], xaxis_title="")
