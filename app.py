...
if "lat" not in st.session_state: st.session_state.lat = 25.3  # Defaulting to Varanasi!
...
def load_climate_data():
    ...
    # 🎯 THE MAP FIX: Unwrap the 0-360 world into a standard -180 to 180 map
    if float(ds.lon.max()) > 180:
        ds.coords['lon'] = (ds.coords['lon'] + 180) % 360 - 180
        ds = ds.sortby(ds.lon) # This stitches the map back together correctly
...
try:
    ...
    trend_fig = px.line(trend_df, x="time", y=param, markers=True, title=f"Historical {param} Trend for selected location")
